...
@router.get("/count")
async def count_users(session: AsyncSession = Depends(get_session)) -> Dict[str, int]:
...
async def check_user_exists(user_id: int, session: AsyncSession = Depends(get_session)):
